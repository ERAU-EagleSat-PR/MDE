...
//*****************************************************************************
void
MRAMSequenceTransmit(unsigned char sequence_start, unsigned char sequence_offset)
{
...
//*****************************************************************************
void
MRAMSequenceRetrieve(unsigned char sequence_start, unsigned char sequence_offset)
{
  ...
  SSIDataPut(SPI_NUM_BASE, MRAM_READ);
  ...
  for(byte_num = 0; byte_num < MRAM_SIZE_BYTES; byte_num++){
    ...
    if(blank_byte_count >= BLANK_BYTE_UNRESPONSIVE_COUNT){
      ...
      chip_unresponsive[MRAM_NUMBER] = 1;
      break;
    }
    ...
    if(wrong_bits != 0){
      ...
      uint32_t shifted_num = MRAM_NUMBER << 20;
  ...
  if(blank_byte_count < BLANK_BYTE_UNRESPONSIVE_COUNT){
    ...
    chip_unresponsive[MRAM_NUMBER] = 0;
  }
