...
WriteToChip(uint32_t chip_number, unsigned char sequence_start, unsigned char sequence_offset){
  ...
  if (chipNumber < 4) {
      FlashSequenceTransmit(sequence_start, sequence_offset, chip_number);
  } else if (chipNumber < 8) {
      FRAMSequenceTransmit(sequence_start, sequence_offset, chip_number)
  } else if (chipNumber < 12) {
      MRAMSequenceTransmit(sequence_start, sequence_offset, chip_number)
  } else {
      SRAMSequenceTransmit(sequence_start, sequence_offset, chip_number)
  }
}
